    ...
    protected List<Action> anotherCubeLeftSwitchToScale(){
        ...
        return Arrays.asList(
                ...
                new ParallelAction(Arrays.asList(
                        ...
                        new ElevatorAction(Elevator.ElevatorPresetEnum.SCALE_HIGH_STACKING))),
                new TimeOutOrHaltedDriveAction(new FollowDynamicPathAction(FollowDynamicPathAction.PathMode.RUNPATHTOTARGET, false, new Translation2d(AutoPaths.LLScale.get(AutoPaths.LLScale.size() - 1).position.getX() - 0, AutoPaths.LLScale.get(AutoPaths.LLScale.size() - 1).position.getY() + 10)), 6),new PickupAction(PickupAction.PickupStateEnum.TIMEDOUTTAKE)
        );
    }
